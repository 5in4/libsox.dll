...
url=http://downloads.xiph.org/releases/ogg/${SOURCE_ARCHIVE}
md5='84a35715170f2cd4c77a4448772b95d5'
...
DEV_CONTENTS="share/aclocal --exclude=*.dll --exclude=*.la lib include"
...
then
  ...
  CFLAGS="$CFLAGS -march=i686" ${srcdir}/configure --prefix=${prefix} 2>&1 | tee -a ${logdir}/configure.log && if ! test "x${PIPESTATUS[0]}" == "x0"; then fail $LINENO; fi
fi
...
then
  ...
  mv -t ${instdir}${prefix}/share/doc/${name}/${ver} ${instdir}${prefix}/share/doc/${name}-${ver}/*
  rm ${instdir}${prefix}/share/doc/${name}-${ver}
