...
url=http://downloads.xiph.org/releases/vorbis/${SOURCE_ARCHIVE}
...
then
  ...
  CFLAGS="$CFLAGS -march=i686" ${srcdir}/configure --prefix=${prefix} --enable-docs --enable-examples 2>&1 | tee -a ${logdir}/configure.log && if ! test "x${PIPESTATUS[0]}" == "x0"; then fail $LINENO; fi
fi
...
then
  ...
  rm -rf ${instdir}${prefix}/share/doc/${name}-${ver}
