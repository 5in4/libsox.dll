...
url=http://www.wavpack.com/${SOURCE_ARCHIVE}
...
DLLPKG=lib${badname}-${ver}-${rev}-${subsys}-dll-${dllver}.tar.lzma
...
DEV_CONTENTS="--exclude=*.dll --exclude=*.la lib include"
...
DOC_CONTENTS="$DOC_CONTENTS $(for i in $licfiles; do echo --exclude=$i; done) share/doc/${capname} share/man"
...
then
  ...
  CPPFLAGS="-D__MSVCRT_VERSION__=0x0601" CFLAGS="$CFLAGS -march=i686" ${srcdir}/configure --prefix=${prefix} 2>&1 | tee -a ${logdir}/configure.log && if ! test "x${PIPESTATUS[0]}" == "x0"; then fail $LINENO; fi
fi
